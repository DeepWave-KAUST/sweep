#include "acoustic_lsrtm3d_cpu.h"
...
namespace sweep_cpu::acoustic_lsrtm3d {
namespace {
...
bool can_use_acoustic_lsrtm3d_raw_forward(const ForwardInput& p)
{
...
bool can_use_acoustic_lsrtm3d_raw_backward(const BackwardInput& p)
{
    if (p.free_surface) return false;
    if (p.models.size() != 2 || p.pml_vals.size() < 9) return false;
    if (p.M < 1) return false;
...
struct AcousticLSRTM3DRawState {
    ...
    AcousticLSRTM3DRawState() = default;

    explicit AcousticLSRTM3DRawState(int64_t total)
        : u_prev(total, 0.0f),
          u_now(total, 0.0f),
          u_next(total, 0.0f),
          psix(total, 0.0f),
...
void zero_acoustic_lsrtm3d_boundary_width(
...
void zero_acoustic_lsrtm3d_boundary(std::vector<float>& field, int64_t B, int64_t nz, int64_t ny, int64_t nx, int M)
{
    zero_acoustic_lsrtm3d_boundary_width(field, B, nz, ny, nx, M);
}

int acoustic_lsrtm3d_phys_x0(int abcn, int M)
{
    return abcn + M;
}

int acoustic_lsrtm3d_phys_x1(int64_t nx, int abcn, int M)
{
    return static_cast<int>(nx) - abcn - M;
}

int acoustic_lsrtm3d_phys_y0(int abcn, int M)
{
    return abcn + M;
}

int acoustic_lsrtm3d_phys_y1(int64_t ny, int abcn, int M)
{
    return static_cast<int>(ny) - abcn - M;
}

int acoustic_lsrtm3d_phys_z0(int abcn, int M, bool free_surface)
{
    return free_surface ? M : abcn + M;
}

int acoustic_lsrtm3d_phys_z1(int64_t nz, int abcn, int M)
{
    return static_cast<int>(nz) - abcn - M;
}

int acoustic_lsrtm3d_checkpoint_index(int it, int nt, int interval)
{
...
int acoustic_lsrtm3d_recursive_checkpoint_index(int it, const std::vector<int>& steps, int& next_idx)
{
...
void save_acoustic_lsrtm3d_checkpoint(
    const std::vector<torch::Tensor>& checkpoints,
    int checkpoint_idx,
    const AcousticLSRTM3DRawState& s
)
{
    if (checkpoint_idx < 0) return;
    TORCH_CHECK(checkpoints.size() == 8, "AcousticLSRTM3D CPU checkpointing expects 8 checkpoint tensors");
...
void load_acoustic_lsrtm3d_checkpoint(
    const std::vector<torch::Tensor>& checkpoints,
    int checkpoint_idx,
    AcousticLSRTM3DRawState& s
)
{
    TORCH_CHECK(checkpoints.size() == 8, "AcousticLSRTM3D CPU checkpointing expects 8 checkpoint tensors");
...
void check_acoustic_lsrtm3d_boundary_tensors(const std::vector<torch::Tensor>& boundary)
{
    TORCH_CHECK(boundary.size() == 6, "AcousticLSRTM3D boundary saving expects 6 boundary tensors");
    for (const auto& t : boundary) {
        TORCH_CHECK(t.is_cpu(), "AcousticLSRTM3D CPU boundary tensors must be on CPU");
        TORCH_CHECK(t.is_contiguous(), "AcousticLSRTM3D CPU boundary tensors must be contiguous");
        TORCH_CHECK(t.scalar_type() == torch::kFloat32, "AcousticLSRTM3D CPU boundary tensors must be float32");
    }
}

void save_acoustic_lsrtm3d_boundary(
...
)
{
    ...
    check_acoustic_lsrtm3d_boundary_tensors(boundary);
    const int x0 = acoustic_lsrtm3d_phys_x0(abcn, M);
    const int x1 = acoustic_lsrtm3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_lsrtm3d_phys_y0(abcn, M);
    const int y1 = acoustic_lsrtm3d_phys_y1(ny, abcn, M);
    const int z0 = acoustic_lsrtm3d_phys_z0(abcn, M, free_surface);
    const int z1 = acoustic_lsrtm3d_phys_z1(nz, abcn, M);
...
void restore_acoustic_lsrtm3d_boundary(
    const std::vector<torch::Tensor>& boundary,
    int saved_it,
    std::vector<float>& u,
    int64_t B,
...
{
    TORCH_CHECK(!boundary.empty(), "AcousticLSRTM3D boundary-saving backward requires saved boundaries");
    check_acoustic_lsrtm3d_boundary_tensors(boundary);
    const int x0 = acoustic_lsrtm3d_phys_x0(abcn, M);
    const int x1 = acoustic_lsrtm3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_lsrtm3d_phys_y0(abcn, M);
...
void write_acoustic_lsrtm3d_boundary_file(
...
void read_acoustic_lsrtm3d_boundary_file(
    const std::string& path,
    size_t offset_elems,
...
void save_acoustic_lsrtm3d_boundary_disk(
...
{
    TORCH_CHECK(files.size() == 6, "AcousticLSRTM3D disk boundary saving expects 6 files");
    const int x0 = acoustic_lsrtm3d_phys_x0(abcn, M);
    const int x1 = acoustic_lsrtm3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_lsrtm3d_phys_y0(abcn, M);
    const int y1 = acoustic_lsrtm3d_phys_y1(ny, abcn, M);
    ...
    write_acoustic_lsrtm3d_boundary_file(files[0], static_cast<size_t>(it) * top_elems, top);
    write_acoustic_lsrtm3d_boundary_file(files[1], static_cast<size_t>(it) * top_elems, bottom);
    write_acoustic_lsrtm3d_boundary_file(files[2], static_cast<size_t>(it) * front_elems, front);
    write_acoustic_lsrtm3d_boundary_file(files[3], static_cast<size_t>(it) * front_elems, back);
    write_acoustic_lsrtm3d_boundary_file(files[4], static_cast<size_t>(it) * left_elems, left);
    write_acoustic_lsrtm3d_boundary_file(files[5], static_cast<size_t>(it) * left_elems, right);
}

void restore_acoustic_lsrtm3d_boundary_disk(
    const std::vector<std::string>& files,
    int saved_it,
    std::vector<float>& u,
    int64_t B,
...
)
{
    ...
    std::vector<float> front(front_elems), back(front_elems);
    std::vector<float> left(left_elems), right(left_elems);
    read_acoustic_lsrtm3d_boundary_file(files[0], static_cast<size_t>(saved_it) * top_elems, top);
    read_acoustic_lsrtm3d_boundary_file(files[1], static_cast<size_t>(saved_it) * top_elems, bottom);
    read_acoustic_lsrtm3d_boundary_file(files[2], static_cast<size_t>(saved_it) * front_elems, front);
    read_acoustic_lsrtm3d_boundary_file(files[3], static_cast<size_t>(saved_it) * front_elems, back);
    read_acoustic_lsrtm3d_boundary_file(files[4], static_cast<size_t>(saved_it) * left_elems, left);
    read_acoustic_lsrtm3d_boundary_file(files[5], static_cast<size_t>(saved_it) * left_elems, right);
...
template <int Order>
void advance_acoustic_lsrtm3d_cpml(
    AcousticLSRTM3DRawState& s,
    const float* vp,
    const float* az,
    const float* bz,
...
template <int Order>
void advance_acoustic_lsrtm3d_coupled_cpml(
    AcousticLSRTM3DRawState& bg,
    AcousticLSRTM3DRawState& sc,
    const float* vp,
    const float* mp,
    const float* az,
    const float* bz,
    ...
    const StencilCoefficients& stencil)
{
    ...
    std::vector<float> bg_utt_buffer;
    float* bg_utt_ptr = bg_utt;
    if (bg_utt_ptr == nullptr) {
        bg_utt_buffer.assign(total, 0.0f);
        bg_utt_ptr = bg_utt_buffer.data();
    }

    advance_acoustic_lsrtm3d_cpml<Order>(
        bg, vp, az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
        B, nz, ny, nx, abcn, free_surface,
        inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, bg_utt_ptr
    , stencil);
    advance_acoustic_lsrtm3d_cpml<Order>(
        sc, vp, az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
        B, nz, ny, nx, abcn, free_surface,
    ...
    at::parallel_for(0, total, 4096, [&](int64_t begin, int64_t end) {
        for (int64_t linear = begin; linear < end; ++linear) {
            sc.u_next[linear] += dt2 * mp[linear] * bg_utt_ptr[linear];
        }
    });
}

template <int Order>
void advance_acoustic_lsrtm3d_nopml(
    AcousticLSRTM3DRawState& s,
    const float* vp,
    int64_t B,
    int64_t nz,
...
void add_acoustic_lsrtm3d_source(
    std::vector<float>& field,
    const float* source,
    const int32_t* loc,
    int64_t B,
...
void record_acoustic_lsrtm3d_receivers(
...
void accumulate_acoustic_lsrtm3d_source_grad(
    float* grad_wavelet,
...
[[maybe_unused]] void accumulate_acoustic_lsrtm3d_imaging(
...
void accumulate_acoustic_lsrtm3d_mp(
    const float* bg_utt,
    const std::vector<float>& adj,
    const float* vp,
    float* grad_mp,
...
template <int Order>
void acoustic_lsrtm3d_adjoint_step(
    AcousticLSRTM3DRawState& adj,
    const float* vp,
    ...
    const StencilCoefficients& stencil)
{
    ...
    , stencil);
    add_acoustic_lsrtm3d_source(adj.u_next, adjoint_source, adjoint_sources, B, adjoint_nsrc, nt, it, nz, ny, nx);
    adj.swap();
    zero_acoustic_lsrtm3d_boundary(adj.u_prev, B, nz, ny, nx, M);
    accumulate_acoustic_lsrtm3d_source_grad(grad_wavelet, adj.u_now, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
}

template <int Order>
CpuForwardResult forward_acoustic_lsrtm3d_raw_impl(const ForwardInput& p)
{
    ...
    TORCH_CHECK(vp_t.size(1) == 1, "AcousticLSRTM3D raw CPU forward expects one model channel");
    TORCH_CHECK(mp_t.sizes() == vp_t.sizes(), "AcousticLSRTM3D raw CPU forward expects vp and mp to have the same shape");
    TORCH_CHECK(source_t.size(0) == B && source_t.size(1) == nsrc && source_t.size(2) == nt,
                "AcousticLSRTM3D raw CPU forward source shape mismatch");
    TORCH_CHECK(nz > 2 * M && ny > 2 * M && nx > 2 * M, "AcousticLSRTM3D raw CPU forward grid is too small");
    auto boundary = active_boundary_tensors(p);
    const int save_width = p.abcn > 0 ? M + 1 : M;
    if (p.use_boundary_saving) {
        if (p.boundary_on_disk) {
            TORCH_CHECK(p.boundary_disk_files.size() == 6, "AcousticLSRTM3D CPU disk boundary-saving forward requires 6 files");
        } else {
            TORCH_CHECK(!boundary.empty(), "AcousticLSRTM3D CPU boundary-saving forward requires boundary tensors");
        }
    }
    if (p.use_checkpoint) {
        TORCH_CHECK(p.checkpoints.size() == 8, "AcousticLSRTM3D CPU checkpointing expects 8 checkpoint tensors");
    }
    ...
    AcousticLSRTM3DRawState bg(B * spatial);
    AcousticLSRTM3DRawState sc(B * spatial);
    ...
    for (int64_t it = 0; it < nt; ++it) {
        float* bg_utt = u_allt.defined() ? u_allt.select(0, it).data_ptr<float>() : nullptr;
        advance_acoustic_lsrtm3d_coupled_cpml<Order>(
            bg, sc, vp, mp, az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
            B, nz, ny, nx, p.abcn, p.free_surface,
            inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, bg_utt
        , stencil);
        if (p.use_boundary_saving) {
            if (p.boundary_on_disk) {
                save_acoustic_lsrtm3d_boundary_disk(
                ...
                save_acoustic_lsrtm3d_boundary(
                    boundary,
                    static_cast<int>(it),
                    bg.u_now,
                    B,
        ...
        add_acoustic_lsrtm3d_source(bg.u_next, source, sources, B, nsrc, nt, it, nz, ny, nx);
        record_acoustic_lsrtm3d_receivers(sc.u_next, rec, receivers, B, nrec, nt, it, nz, ny, nx);
        bg.swap();
        sc.swap();
        zero_acoustic_lsrtm3d_boundary(bg.u_prev, B, nz, ny, nx, M);
        zero_acoustic_lsrtm3d_boundary(sc.u_prev, B, nz, ny, nx, M);
        if (p.use_checkpoint) {
            const int ckpt_idx = p.use_recursive_checkpoint
                ? acoustic_lsrtm3d_recursive_checkpoint_index(static_cast<int>(it), recursive_steps, next_recursive_checkpoint)
                : acoustic_lsrtm3d_checkpoint_index(static_cast<int>(it), static_cast<int>(nt), p.checkpoint_interval);
            save_acoustic_lsrtm3d_checkpoint(p.checkpoints, ckpt_idx, bg);
        }
    }
    ...
    if (p.use_boundary_saving) {
        ...
        TORCH_CHECK(last_two.is_contiguous(), "AcousticLSRTM3D CPU boundary-saving last_two must be contiguous");
...
CpuForwardResult forward_acoustic_lsrtm3d_raw(const ForwardInput& p)
{
    SWEEP_CPU_DISPATCH_STENCIL(p.M, forward_acoustic_lsrtm3d_raw_impl, p);
}

template <int Order>
BackwardOutput backward_acoustic_lsrtm3d_full_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.u_forward.defined() && p.u_forward.numel() == nt * total,
                "AcousticLSRTM3D full CPU backward requires saved forward wavefields");
    TORCH_CHECK(p.u_forward.is_contiguous(), "AcousticLSRTM3D full CPU forward wavefields must be contiguous");
    ...
    for (int64_t it = nt - 1; it >= 0; --it) {
        ...
        , stencil);
        accumulate_acoustic_lsrtm3d_mp(
...
template <int Order>
BackwardOutput backward_acoustic_lsrtm3d_ckpt_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.checkpoints.size() == 8, "AcousticLSRTM3D checkpoint CPU backward expects 8 checkpoint tensors");
    ...
    AcousticLSRTM3DRawState adj(total);
    AcousticLSRTM3DRawState fwd(total);
    ...
    for (int chunk_id = num_chunks - 1; chunk_id >= 0; --chunk_id) {
        ...
        load_acoustic_lsrtm3d_checkpoint(p.checkpoints, chunk_id, fwd);
        ...
        for (int it = start; it < end; ++it) {
            ...
            , stencil);
            add_acoustic_lsrtm3d_source(fwd.u_next, forward_source, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
            fwd.swap();
            zero_acoustic_lsrtm3d_boundary(fwd.u_prev, B, nz, ny, nx, M);
        }
...
template <int Order>
BackwardOutput backward_acoustic_lsrtm3d_recursive_ckpt_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.checkpoints.size() == 8, "AcousticLSRTM3D recursive checkpoint CPU backward expects 8 checkpoint tensors");
    ...
    auto advance_interval = [&](AcousticLSRTM3DRawState& state, int start, int end) {
        for (int it = start; it < end; ++it) {
            advance_acoustic_lsrtm3d_cpml<Order>(
                ...
                inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, nullptr
            , stencil);
            add_acoustic_lsrtm3d_source(state.u_next, forward_source, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
            state.swap();
            zero_acoustic_lsrtm3d_boundary(state.u_prev, B, nz, ny, nx, M);
        }
    };

    std::function<void(int, int, AcousticLSRTM3DRawState&)> process_interval =
        [&](int start, int end, AcousticLSRTM3DRawState& start_state) {
            if (start >= end) return;
            if (end - start == 1) {
                advance_acoustic_lsrtm3d_cpml<Order>(
                ...
                , stencil);
                add_acoustic_lsrtm3d_source(start_state.u_next, forward_source, forward_sources, B, forward_nsrc, nt, start, nz, ny, nx);
                start_state.swap();
                zero_acoustic_lsrtm3d_boundary(start_state.u_prev, B, nz, ny, nx, M);
            ...
            AcousticLSRTM3DRawState mid_state = start_state;
    ...
    for (int segment_idx = static_cast<int>(steps.size()); segment_idx >= 0; --segment_idx) {
        ...
        AcousticLSRTM3DRawState start_state(total);
        if (segment_idx > 0) {
            load_acoustic_lsrtm3d_checkpoint(p.checkpoints, segment_idx - 1, start_state);
        }
...
template <int Order>
BackwardOutput backward_acoustic_lsrtm3d_bs_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.u_last_two.defined() && p.u_last_two.numel() >= 2 * total,
                "AcousticLSRTM3D boundary-saving CPU backward requires last_two");
    auto boundary = active_boundary_tensors(p);
    if (p.boundary_on_disk) {
        TORCH_CHECK(p.boundary_disk_files.size() == 6, "AcousticLSRTM3D CPU disk boundary-saving backward requires 6 files");
    } else {
        TORCH_CHECK(!boundary.empty(), "AcousticLSRTM3D boundary-saving CPU backward requires boundary tensors");
    }
    ...
    zero_acoustic_lsrtm3d_boundary_width(fwd.u_prev, B, nz, ny, nx, p.abcn + M);
    zero_acoustic_lsrtm3d_boundary_width(fwd.u_now, B, nz, ny, nx, p.abcn + M);
    ...
    for (int64_t it = nt - 1; it >= 1; --it) {
        ...
        advance_acoustic_lsrtm3d_nopml<Order>(fwd, vp, B, nz, ny, nx, p.abcn, p.free_surface, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
        if (p.boundary_on_disk) {
            restore_acoustic_lsrtm3d_boundary_disk(
                p.boundary_disk_files,
            ...
            restore_acoustic_lsrtm3d_boundary(
                boundary,
                static_cast<int>(it - 1),
                fwd.u_next,
                B,
...
template <int Order>
BackwardOutput backward_acoustic_lsrtm3d_recompute_impl(const BackwardInput& p)
{
    auto vp_t = p.models[0];
    const int64_t B = vp_t.size(0);
    const int64_t nz = vp_t.size(2);
    ...
    auto forward_result = forward_acoustic_lsrtm3d_raw(fwd);

    BackwardInput replay = p;
    replay.u_forward = forward_result.wavefield;
    TORCH_CHECK(replay.u_forward.numel() == nt * total, "AcousticLSRTM3D recompute failed to save forward wavefields");
    return backward_acoustic_lsrtm3d_full_impl<Order>(replay);
}

BackwardOutput backward_acoustic_lsrtm3d_raw(const BackwardInput& p)
{
    ...
    #define SWEEP_CPU_LSRTM3D_BACKWARD_CASE(ORDER) \
        if (has_full_wavefield) return backward_acoustic_lsrtm3d_full_impl<ORDER>(p); \
        if (has_boundary_state) return backward_acoustic_lsrtm3d_bs_impl<ORDER>(p); \
        if (has_checkpoints && has_recursive_steps) return backward_acoustic_lsrtm3d_recursive_ckpt_impl<ORDER>(p); \
        if (has_checkpoints) return backward_acoustic_lsrtm3d_ckpt_impl<ORDER>(p); \
        return backward_acoustic_lsrtm3d_recompute_impl<ORDER>(p);
    SWEEP_CPU_DISPATCH_STENCIL_BODY(p.M, SWEEP_CPU_LSRTM3D_BACKWARD_CASE);
    #undef SWEEP_CPU_LSRTM3D_BACKWARD_CASE
}
...
{
    TORCH_CHECK(engine::is_cpu_input(in), "sweep_cpu::acoustic_lsrtm3d::forward called with non-CPU tensors");
    if (!can_use_acoustic_lsrtm3d_raw_forward(in)) {
        return engine::forward(in, EquationKind::AcousticLSRTM3D);
    }
    auto result = forward_acoustic_lsrtm3d_raw(in);
    ForwardOutput out;
...
{
    TORCH_CHECK(engine::is_cpu_input(in), "sweep_cpu::acoustic_lsrtm3d::backward called with non-CPU tensors");
    TORCH_CHECK(
        can_use_acoustic_lsrtm3d_raw_backward(in),
        "AcousticLSRTM3D CPU backward requires the handwritten raw float32 path; unsupported inputs will not fall back to torch autograd."
    );
    return backward_acoustic_lsrtm3d_raw(in);
}
