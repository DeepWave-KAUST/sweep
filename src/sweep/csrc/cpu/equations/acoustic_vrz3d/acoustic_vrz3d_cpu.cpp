...
namespace sweep_cpu::acoustic_vrz3d {
namespace {
...
bool can_use_acoustic_vrz3d_raw_forward(const ForwardInput& p)
{
    if (p.free_surface) return false;
    if (p.models.size() != 2 || p.pml_vals.size() < 9) return false;
    ...
    if (!p.models[1].is_contiguous() || p.models[1].scalar_type() != torch::kFloat32 || p.models[1].dim() != 5) return false;
...
bool can_use_acoustic_vrz3d_raw_backward(const BackwardInput& p)
{
    if (p.free_surface) return false;
    if (p.models.size() != 2 || p.pml_vals.size() < 9) return false;
    if (p.M < 1) return false;
...
struct AcousticVRZ3DRawState {
    ...
    AcousticVRZ3DRawState() = default;

    explicit AcousticVRZ3DRawState(int64_t total)
        : u_prev(total, 0.0f),
          u_now(total, 0.0f),
          u_next(total, 0.0f),
          psix(total, 0.0f),
...
void zero_acoustic_vrz3d_boundary_width(
...
void zero_acoustic_vrz3d_boundary(std::vector<float>& field, int64_t B, int64_t nz, int64_t ny, int64_t nx, int M)
{
    zero_acoustic_vrz3d_boundary_width(field, B, nz, ny, nx, M);
}

int acoustic_vrz3d_phys_x0(int abcn, int M)
{
    (void)M;
    return abcn;
}

int acoustic_vrz3d_phys_x1(int64_t nx, int abcn, int M)
{
    (void)M;
    return static_cast<int>(nx) - abcn;
}

int acoustic_vrz3d_phys_y0(int abcn, int M)
{
    (void)M;
    return abcn;
}

int acoustic_vrz3d_phys_y1(int64_t ny, int abcn, int M)
{
    (void)M;
    return static_cast<int>(ny) - abcn;
}

int acoustic_vrz3d_phys_z0(int abcn, int M, bool free_surface)
{
    (void)M;
    return free_surface ? 0 : abcn;
}

int acoustic_vrz3d_phys_z1(int64_t nz, int abcn, int M)
{
    (void)M;
    return static_cast<int>(nz) - abcn;
}

int acoustic_vrz3d_checkpoint_index(int it, int nt, int interval)
{
...
int acoustic_vrz3d_recursive_checkpoint_index(int it, const std::vector<int>& steps, int& next_idx)
{
...
void save_acoustic_vrz3d_checkpoint(
    const std::vector<torch::Tensor>& checkpoints,
    int checkpoint_idx,
    const AcousticVRZ3DRawState& s
)
{
    if (checkpoint_idx < 0) return;
    TORCH_CHECK(checkpoints.size() == 8, "AcousticVRZ3D CPU checkpointing expects 8 checkpoint tensors");
...
void load_acoustic_vrz3d_checkpoint(
    const std::vector<torch::Tensor>& checkpoints,
    int checkpoint_idx,
    AcousticVRZ3DRawState& s
)
{
    TORCH_CHECK(checkpoints.size() == 8, "AcousticVRZ3D CPU checkpointing expects 8 checkpoint tensors");
...
void check_acoustic_vrz3d_boundary_tensors(const std::vector<torch::Tensor>& boundary)
{
    TORCH_CHECK(boundary.size() == 6, "AcousticVRZ3D boundary saving expects 6 boundary tensors");
    for (const auto& t : boundary) {
        TORCH_CHECK(t.is_cpu(), "AcousticVRZ3D CPU boundary tensors must be on CPU");
        TORCH_CHECK(t.is_contiguous(), "AcousticVRZ3D CPU boundary tensors must be contiguous");
        TORCH_CHECK(t.scalar_type() == torch::kFloat32, "AcousticVRZ3D CPU boundary tensors must be float32");
    }
}

void save_acoustic_vrz3d_boundary(
...
)
{
    ...
    check_acoustic_vrz3d_boundary_tensors(boundary);
    const int x0 = acoustic_vrz3d_phys_x0(abcn, M);
    const int x1 = acoustic_vrz3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_vrz3d_phys_y0(abcn, M);
    const int y1 = acoustic_vrz3d_phys_y1(ny, abcn, M);
    const int z0 = acoustic_vrz3d_phys_z0(abcn, M, free_surface);
    const int z1 = acoustic_vrz3d_phys_z1(nz, abcn, M);
...
void restore_acoustic_vrz3d_boundary(
    const std::vector<torch::Tensor>& boundary,
    int saved_it,
    std::vector<float>& u,
    int64_t B,
...
{
    TORCH_CHECK(!boundary.empty(), "AcousticVRZ3D boundary-saving backward requires saved boundaries");
    check_acoustic_vrz3d_boundary_tensors(boundary);
    const int x0 = acoustic_vrz3d_phys_x0(abcn, M);
    const int x1 = acoustic_vrz3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_vrz3d_phys_y0(abcn, M);
...
void write_acoustic_vrz3d_boundary_file(
...
void read_acoustic_vrz3d_boundary_file(
    const std::string& path,
    size_t offset_elems,
...
void save_acoustic_vrz3d_boundary_disk(
...
{
    TORCH_CHECK(files.size() == 6, "AcousticVRZ3D disk boundary saving expects 6 files");
    const int x0 = acoustic_vrz3d_phys_x0(abcn, M);
    const int x1 = acoustic_vrz3d_phys_x1(nx, abcn, M);
    const int y0 = acoustic_vrz3d_phys_y0(abcn, M);
    const int y1 = acoustic_vrz3d_phys_y1(ny, abcn, M);
    ...
    write_acoustic_vrz3d_boundary_file(files[0], static_cast<size_t>(it) * top_elems, top);
    write_acoustic_vrz3d_boundary_file(files[1], static_cast<size_t>(it) * top_elems, bottom);
    write_acoustic_vrz3d_boundary_file(files[2], static_cast<size_t>(it) * front_elems, front);
    write_acoustic_vrz3d_boundary_file(files[3], static_cast<size_t>(it) * front_elems, back);
    write_acoustic_vrz3d_boundary_file(files[4], static_cast<size_t>(it) * left_elems, left);
    write_acoustic_vrz3d_boundary_file(files[5], static_cast<size_t>(it) * left_elems, right);
}

void restore_acoustic_vrz3d_boundary_disk(
    const std::vector<std::string>& files,
    int saved_it,
    std::vector<float>& u,
    int64_t B,
...
)
{
    ...
    std::vector<float> front(front_elems), back(front_elems);
    std::vector<float> left(left_elems), right(left_elems);
    read_acoustic_vrz3d_boundary_file(files[0], static_cast<size_t>(saved_it) * top_elems, top);
    read_acoustic_vrz3d_boundary_file(files[1], static_cast<size_t>(saved_it) * top_elems, bottom);
    read_acoustic_vrz3d_boundary_file(files[2], static_cast<size_t>(saved_it) * front_elems, front);
    read_acoustic_vrz3d_boundary_file(files[3], static_cast<size_t>(saved_it) * front_elems, back);
    read_acoustic_vrz3d_boundary_file(files[4], static_cast<size_t>(saved_it) * left_elems, left);
    read_acoustic_vrz3d_boundary_file(files[5], static_cast<size_t>(saved_it) * left_elems, right);
...
template <int Order>
void advance_acoustic_vrz3d_cpml(
    AcousticVRZ3DRawState& s,
    const float* vp,
    const float* z_model,
    ...
    const StencilCoefficients& stencil)
{
    ...
    at::parallel_for(0, plane_count, 1, [&](int64_t begin, int64_t end) {
        for (int64_t plane = begin; plane < end; ++plane) {
            const int64_t b = plane / ((nz - 2 * M) * (ny - 2 * M));
            const int64_t rem = plane - b * ((nz - 2 * M) * (ny - 2 * M));
            ...
            const float ay_y = ay[y];
            const float by_y = by[y];
            const float dbydy_y = dbydy[y];
            ...
            for (int64_t x = M; x < nx - M; ++x) {
                ...
                const float dvpdz = centered_grad_value<Order>(vp, idx, stride_z, inv_dz, stencil);
                const float dvpdy = centered_grad_value<Order>(vp, idx, stride_y, inv_dy, stencil);
                const float dvpdx = centered_grad_value<Order>(vp, idx, 1, inv_dx, stencil);
                const float z1z = centered_grad_value<Order>(inv_z_model, idx, stride_z, inv_dz, stencil);
                const float z1y = centered_grad_value<Order>(inv_z_model, idx, stride_y, inv_dy, stencil);
                ...
                const float tmpy = (1.0f + by_y) * lap_y + dbydy_y * dudy + daydy * psiy_ptr[idx] + ay_y * dpsiydy;
...
template <int Order>
void advance_acoustic_vrz3d_nopml(
    AcousticVRZ3DRawState& s,
    const float* vp,
    const float* z_model,
    const float* inv_z_model,
    ...
    const StencilCoefficients& stencil)
{
    ...
    const int64_t spatial = nz * ny * nx;
    const int64_t total = B * spatial;
    const int x_start = acoustic_vrz3d_phys_x0(abcn, M) + 1;
    const int x_end = acoustic_vrz3d_phys_x1(nx, abcn, M) - 1;
    const int y_start = acoustic_vrz3d_phys_y0(abcn, M) + 1;
    const int y_end = acoustic_vrz3d_phys_y1(ny, abcn, M) - 1;
    const int z_start = acoustic_vrz3d_phys_z0(abcn, M, free_surface) + 1;
    const int z_end = acoustic_vrz3d_phys_z1(nz, abcn, M) - 1;
    const float* u_prev_ptr = s.u_prev.data();
    const float* u_now_ptr = s.u_now.data();
    float* u_next_ptr = s.u_next.data();
    at::parallel_for(0, total, 65536, [&](int64_t begin, int64_t end) {
    ...
    const int64_t plane_count = B * (z_end - z_start) * (y_end - y_start);
    at::parallel_for(0, plane_count, 1, [&](int64_t begin, int64_t end) {
        for (int64_t plane = begin; plane < end; ++plane) {
            const int64_t b = plane / ((z_end - z_start) * (y_end - y_start));
            const int64_t rem = plane - b * ((z_end - z_start) * (y_end - y_start));
            const int64_t z = z_start + rem / (y_end - y_start);
            const int64_t y = y_start + rem % (y_end - y_start);
            const int64_t base = b * spatial + z * stride_z + y * nx;
            for (int64_t x = x_start; x < x_end; ++x) {
                ...
                const float dbdy = dvpdy * inv_z0 + v * z1y;
                const float dbdx = dvpdx * inv_z0 + v * z1x;
                const float rhs = kappa * (beta * (lap_x + lap_y + lap_z) + dbdx * dudx + dbdy * dudy + dbdz * dudz);
                u_next_ptr[idx] = 2.0f * u_now_ptr[idx] - u_prev_ptr[idx] + dt2 * rhs;
            }
        }
    });
...
void add_acoustic_vrz3d_source(
    std::vector<float>& field,
    const float* source,
    const int32_t* loc,
    int64_t B,
    ...
    for (int64_t b = 0; b < B; ++b) {
        ...
        for (int64_t isrc = 0; isrc < nsrc; ++isrc) {
            ...
            if (x >= 0 && x < nx && y >= 0 && y < ny && z >= 0 && z < nz) {
                base[z * ny * nx + y * nx + x] += scale * source[(b * nsrc + isrc) * nt + it];
            }
        }
    }
}

void record_acoustic_vrz3d_receivers(
...
template <int Order>
void build_acoustic_vrz3d_kappa_lambda(
...
template <int Order>
void accumulate_acoustic_vrz3d_model_grad(
    const float* f_u_now,
    const std::vector<float>& lambda_now,
    ...
    const StencilCoefficients& stencil)
{
    ...
    const int64_t spatial = nz * ny * nx;
    const int64_t plane_count = B * (nz - 2 * M) * (ny - 2 * M);
    const int phys_x0 = acoustic_vrz3d_phys_x0(abcn, M);
    const int phys_x1 = acoustic_vrz3d_phys_x1(nx, abcn, M);
    const int phys_y0 = acoustic_vrz3d_phys_y0(abcn, M);
    const int phys_y1 = acoustic_vrz3d_phys_y1(ny, abcn, M);
    const int phys_z0 = acoustic_vrz3d_phys_z0(abcn, M, free_surface);
    const int phys_z1 = acoustic_vrz3d_phys_z1(nz, abcn, M);
    ...
    at::parallel_for(0, plane_count, 1, [&](int64_t begin, int64_t end) {
        for (int64_t plane = begin; plane < end; ++plane) {
            ...
            for (int64_t x = M; x < nx - M; ++x) {
                const int64_t idx = base + x;
                const float lap_z = laplace_value<Order>(f_u_now, idx, stride_z, inv_dz2, stencil);
                const float lap_y = laplace_value<Order>(f_u_now, idx, stride_y, inv_dy2, stencil);
                const float lap_x = laplace_value<Order>(f_u_now, idx, 1, inv_dx2, stencil);
                const float dpdz = centered_grad_value<Order>(f_u_now, idx, stride_z, inv_dz, stencil);
                const float dpdy = centered_grad_value<Order>(f_u_now, idx, stride_y, inv_dy, stencil);
                ...
                const float dbdy = dvpdy * inv_z0 + v * z1y;
                const float dbdx = dvpdx * inv_z0 + v * z1x;
                const float div_b_grad_p = beta * (lap_x + lap_y + lap_z) + dbdx * dpdx + dbdy * dpdy + dbdz * dpdz;
                float d_q_dpdx = 0.0f;
                float d_q_dpdy = 0.0f;
                float d_q_dpdz = 0.0f;
                if (x >= phys_x0 && x < phys_x1 && y >= phys_y0 && y < phys_y1 && z >= phys_z0 && z < phys_z1) {
                    d_q_dpdx = centered_grad_value<Order>(q, idx, 1, inv_dx, stencil) * dpdx;
                    d_q_dpdy = centered_grad_value<Order>(q, idx, stride_y, inv_dy, stencil) * dpdy;
                    d_q_dpdz = centered_grad_value<Order>(q, idx, stride_z, inv_dz, stencil) * dpdz;
                }
                const float g_kappa = -dt2 * lambda[idx] * div_b_grad_p;
                const float g_beta = dt2 * (d_q_dpdx + d_q_dpdy + d_q_dpdz);
...
template <int Order>
void acoustic_vrz3d_adjoint_step(
    AcousticVRZ3DRawState& adj,
    const float* vp,
    const float* z_model,
    const float* inv_z_model,
    ...
    const StencilCoefficients& stencil)
{
    ...
    advance_acoustic_vrz3d_cpml<Order>(
        adj, vp, z_model, inv_z_model, az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
        B, nz, ny, nx, inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2
    , stencil);
    add_acoustic_vrz3d_source(adj.u_next, adjoint_source, adjoint_sources, B, adjoint_nsrc, nt, it, nz, ny, nx, -1.0f);
    adj.swap();
    zero_acoustic_vrz3d_boundary(adj.u_prev, B, nz, ny, nx, M);
}
...
template <int Order>
CpuForwardResult forward_acoustic_vrz3d_raw_impl(const ForwardInput& p)
{
    ...
    TORCH_CHECK(vp_t.size(1) == 1, "AcousticVRZ3D raw CPU forward expects one model channel");
    TORCH_CHECK(z_t.sizes() == vp_t.sizes(), "AcousticVRZ3D raw CPU forward expects vp and z to have the same shape");
    TORCH_CHECK(source_t.size(0) == B && source_t.size(1) == nsrc && source_t.size(2) == nt,
                "AcousticVRZ3D raw CPU forward source shape mismatch");
    TORCH_CHECK(nz > 2 * M && ny > 2 * M && nx > 2 * M, "AcousticVRZ3D raw CPU forward grid is too small");
    auto boundary = active_boundary_tensors(p);
    const int save_width = M + 1;
    if (p.use_boundary_saving) {
        if (p.boundary_on_disk) {
            TORCH_CHECK(p.boundary_disk_files.size() == 6, "AcousticVRZ3D CPU disk boundary-saving forward requires 6 files");
        } else {
            TORCH_CHECK(!boundary.empty(), "AcousticVRZ3D CPU boundary-saving forward requires boundary tensors");
        }
    }
    if (p.use_checkpoint) TORCH_CHECK(p.checkpoints.size() == 8, "AcousticVRZ3D CPU checkpointing expects 8 checkpoint tensors");
    auto record = torch::empty({B, nrec, nt}, vp_t.options());
    torch::Tensor u_allt;
    if (p.save_all_wavefields) u_allt = torch::zeros({nt, 7, B, nz, ny, nx}, vp_t.options());
    const float* vp = vp_t.data_ptr<float>();
    const float* z_model = z_t.data_ptr<float>();
    auto inv_z = make_inv_z_vector<ForwardInput>(z_t);
    ...
    AcousticVRZ3DRawState state(B * spatial);
    ...
    for (int64_t it = 0; it < nt; ++it) {
        advance_acoustic_vrz3d_cpml<Order>(state, vp, z_model, inv_z.data(), az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                      B, nz, ny, nx, inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
        if (p.use_boundary_saving) {
            if (p.boundary_on_disk) {
                save_acoustic_vrz3d_boundary_disk(p.boundary_disk_files, static_cast<int>(it), state.u_now,
                                                  B, nz, ny, nx, M, p.abcn, p.free_surface, save_width);
            } else {
                save_acoustic_vrz3d_boundary(boundary, static_cast<int>(it), state.u_now,
                                             B, nz, ny, nx, M, p.abcn, p.free_surface, save_width);
            }
        }
        add_acoustic_vrz3d_source(state.u_next, source, sources, B, nsrc, nt, it, nz, ny, nx);
        record_acoustic_vrz3d_receivers(state.u_next, rec, receivers, B, nrec, nt, it, nz, ny, nx);
        state.swap();
        if (u_allt.defined()) {
            copy_vector_to_tensor(state.u_now, u_allt.select(0, it).select(0, 0));
            copy_vector_to_tensor(state.psix, u_allt.select(0, it).select(0, 1));
            copy_vector_to_tensor(state.psiy, u_allt.select(0, it).select(0, 2));
            copy_vector_to_tensor(state.psiz, u_allt.select(0, it).select(0, 3));
            copy_vector_to_tensor(state.zetax, u_allt.select(0, it).select(0, 4));
            copy_vector_to_tensor(state.zetay, u_allt.select(0, it).select(0, 5));
            copy_vector_to_tensor(state.zetaz, u_allt.select(0, it).select(0, 6));
        }
        if (p.use_checkpoint) {
            const int ckpt_idx = p.use_recursive_checkpoint
                ? acoustic_vrz3d_recursive_checkpoint_index(static_cast<int>(it), recursive_steps, next_recursive_checkpoint)
                : acoustic_vrz3d_checkpoint_index(static_cast<int>(it), static_cast<int>(nt), p.checkpoint_interval);
            save_acoustic_vrz3d_checkpoint(p.checkpoints, ckpt_idx, state);
        }
    }
    torch::Tensor last_two = p.last_two.defined() ? p.last_two : torch::empty({0}, vp_t.options());
    if (p.use_boundary_saving) {
        if (!last_two.defined() || last_two.numel() == 0) last_two = torch::zeros({1, 2, B, 1, nz, ny, nx}, vp_t.options());
        TORCH_CHECK(last_two.is_contiguous(), "AcousticVRZ3D CPU boundary-saving last_two must be contiguous");
...
CpuForwardResult forward_acoustic_vrz3d_raw(const ForwardInput& p)
{
    SWEEP_CPU_DISPATCH_STENCIL(p.M, forward_acoustic_vrz3d_raw_impl, p);
}

template <int Order>
BackwardOutput backward_acoustic_vrz3d_full_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    const int64_t nt = static_cast<int64_t>(p.nt);
    TORCH_CHECK(p.u_forward.defined() && p.u_forward.dim() == 6 && p.u_forward.size(0) == nt && p.u_forward.size(1) == 7,
                "AcousticVRZ3D full CPU backward requires saved forward wavefields with shape (nt, 7, B, nz, ny, nx)");
    TORCH_CHECK(p.u_forward.is_contiguous(), "AcousticVRZ3D full CPU forward wavefields must be contiguous");
    ...
    const float inv_dx2 = inv_dx * inv_dx;
    const float dt2 = static_cast<float>(p.dt * p.dt);
    AcousticVRZ3DRawState adj(total);
    std::vector<float> kappa_lambda(total, 0.0f);
    for (int64_t it = nt - 1; it >= 0; --it) {
        acoustic_vrz3d_adjoint_step<Order>(adj, vp, z_model, inv_z.data(), adjoint_source, adjoint_sources,
                                        az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                        B, nz, ny, nx, adjoint_nsrc, nt, it,
                                        inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
        build_acoustic_vrz3d_kappa_lambda<Order>(adj.u_now, vp, z_model, kappa_lambda, total);
        const float* fwd_u = p.u_forward.select(0, it).select(0, 0).data_ptr<float>();
        accumulate_acoustic_vrz3d_model_grad<Order>(fwd_u, adj.u_now, kappa_lambda, vp, z_model, inv_z.data(), grad_vp_ptr, grad_z_ptr,
...
template <int Order>
BackwardOutput backward_acoustic_vrz3d_ckpt_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.checkpoints.size() == 8, "AcousticVRZ3D checkpoint CPU backward expects 8 checkpoint tensors");
    auto grad_vp = torch::zeros_like(vp_t);
    auto grad_z = torch::zeros_like(z_t);
    const float* vp = vp_t.data_ptr<float>();
    const float* z_model = z_t.data_ptr<float>();
    ...
    const float dt2 = static_cast<float>(p.dt * p.dt);
    AcousticVRZ3DRawState adj(total);
    AcousticVRZ3DRawState fwd(total);
    ...
    for (int chunk_id = num_chunks - 1; chunk_id >= 0; --chunk_id) {
        ...
        load_acoustic_vrz3d_checkpoint(p.checkpoints, chunk_id, fwd);
        for (int it = start; it < end; ++it) {
            advance_acoustic_vrz3d_cpml<Order>(fwd, vp, z_model, inv_z.data(), az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                          B, nz, ny, nx, inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
            add_acoustic_vrz3d_source(fwd.u_next, forward_source, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
            fwd.swap();
        ...
        for (int it = end - 1; it >= start; --it) {
            ...
            accumulate_acoustic_vrz3d_model_grad<Order>(fwd_ptr, adj.u_now, kappa_lambda, vp, z_model, inv_z.data(), grad_vp_ptr, grad_z_ptr,
                                                    B, nz, ny, nx, p.abcn, p.free_surface,
                                                    inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
        }
    }
...
template <int Order>
BackwardOutput backward_acoustic_vrz3d_recursive_ckpt_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.checkpoints.size() == 8, "AcousticVRZ3D recursive checkpoint CPU backward expects 8 checkpoint tensors");
    auto grad_vp = torch::zeros_like(vp_t);
    auto grad_z = torch::zeros_like(z_t);
    const float* vp = vp_t.data_ptr<float>();
    const float* z_model = z_t.data_ptr<float>();
    ...
    std::vector<float> kappa_lambda(total, 0.0f);
    auto advance_interval = [&](AcousticVRZ3DRawState& state, int start, int end) {
        for (int it = start; it < end; ++it) {
            advance_acoustic_vrz3d_cpml<Order>(state, vp, z_model, inv_z.data(), az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                          B, nz, ny, nx, inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
            add_acoustic_vrz3d_source(state.u_next, forward_source, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
            state.swap();
        }
    };
    std::function<void(int, int, AcousticVRZ3DRawState&)> process_interval =
        [&](int start, int end, AcousticVRZ3DRawState& start_state) {
            if (start >= end) return;
            if (end - start == 1) {
                advance_acoustic_vrz3d_cpml<Order>(start_state, vp, z_model, inv_z.data(), az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                              B, nz, ny, nx, inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
                add_acoustic_vrz3d_source(start_state.u_next, forward_source, forward_sources, B, forward_nsrc, nt, start, nz, ny, nx);
                start_state.swap();
                std::copy(start_state.u_now.begin(), start_state.u_now.end(), fwd_now.begin());
                acoustic_vrz3d_adjoint_step<Order>(adj, vp, z_model, inv_z.data(), adjoint_source, adjoint_sources,
                                                az, bz, dbzdz, ay, by, dbydy, ax, bx, dbxdx,
                                                B, nz, ny, nx, adjoint_nsrc, nt, start,
                                                inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
                build_acoustic_vrz3d_kappa_lambda<Order>(adj.u_now, vp, z_model, kappa_lambda, total);
                accumulate_acoustic_vrz3d_model_grad<Order>(fwd_now.data(), adj.u_now, kappa_lambda, vp, z_model, inv_z.data(), grad_vp_ptr, grad_z_ptr,
                                                        B, nz, ny, nx, p.abcn, p.free_surface,
                                                        inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2, stencil);
                return;
            }
            const int mid = start + (end - start) / 2;
            AcousticVRZ3DRawState mid_state = start_state;
    ...
    for (int segment_idx = static_cast<int>(steps.size()); segment_idx >= 0; --segment_idx) {
        ...
        AcousticVRZ3DRawState start_state(total);
        if (segment_idx > 0) load_acoustic_vrz3d_checkpoint(p.checkpoints, segment_idx - 1, start_state);
...
template <int Order>
BackwardOutput backward_acoustic_vrz3d_raw_impl(const BackwardInput& p);

template <int Order>
BackwardOutput backward_acoustic_vrz3d_bs_impl(const BackwardInput& p)
{
    const StencilCoefficients stencil{p.M, p.lap_coes.data_ptr<float>(), p.grad_coes.data_ptr<float>()};
    [[maybe_unused]] const int M = sweep_cpu::ops::stencil_half_order<Order>(stencil);
    ...
    TORCH_CHECK(p.u_last_two.defined() && p.u_last_two.numel() > 0,
                "AcousticVRZ3D CPU boundary-saving backward requires u_last_two");
    if (p.boundary_on_disk) {
        TORCH_CHECK(p.boundary_disk_files.size() == 6,
                    "AcousticVRZ3D CPU disk boundary-saving backward requires 6 files");
    } else {
        TORCH_CHECK(!active_boundary_tensors(p).empty(),
                    "AcousticVRZ3D CPU boundary-saving backward requires saved boundary tensors");
    }
    ...
    for (int64_t it = nt - 1; it >= 1; --it) {
        acoustic_vrz3d_adjoint_step<Order>(
        ...
        advance_acoustic_vrz3d_nopml<Order>(
            fwd, vp, z_model, inv_z.data(), B, nz, ny, nx, p.abcn, p.free_surface,
            inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2
        , stencil);
        add_acoustic_vrz3d_source(fwd.u_next, forward_source, forward_sources, B, forward_nsrc, nt, it, nz, ny, nx);
        if (p.boundary_on_disk) {
            restore_acoustic_vrz3d_boundary_disk(
                p.boundary_disk_files, static_cast<int>(it), fwd.u_next,
                B, nz, ny, nx, M, p.abcn, p.free_surface, save_width
            );
        } else {
            restore_acoustic_vrz3d_boundary(
        ...
        build_acoustic_vrz3d_kappa_lambda<Order>(adj.u_now, vp, z_model, kappa_lambda, total);
        accumulate_acoustic_vrz3d_model_grad<Order>(
            fwd.u_now.data(), adj.u_now, kappa_lambda, vp, z_model, inv_z.data(), grad_vp_ptr, grad_z_ptr,
            B, nz, ny, nx, p.abcn, p.free_surface,
            inv_dz, inv_dy, inv_dx, inv_dz2, inv_dy2, inv_dx2, dt2
        , stencil);
    }
...
BackwardOutput backward_acoustic_vrz3d_raw_impl(const BackwardInput& p)
{
    ...
    auto forward_result = forward_acoustic_vrz3d_raw(fwd);
    BackwardInput replay = p;
    replay.u_forward = forward_result.wavefield;
    return backward_acoustic_vrz3d_full_impl<Order>(replay);
}

BackwardOutput backward_acoustic_vrz3d_raw(const BackwardInput& p)
{
    ...
    #define SWEEP_CPU_VRZ3D_BACKWARD_CASE(ORDER) \
        if (has_full_wavefield) return backward_acoustic_vrz3d_full_impl<ORDER>(p); \
        if (has_boundary_state) return backward_acoustic_vrz3d_bs_impl<ORDER>(p); \
        if (has_checkpoints && has_recursive_steps) return backward_acoustic_vrz3d_recursive_ckpt_impl<ORDER>(p); \
        if (has_checkpoints) return backward_acoustic_vrz3d_ckpt_impl<ORDER>(p); \
        return backward_acoustic_vrz3d_raw_impl<ORDER>(p);
    SWEEP_CPU_DISPATCH_STENCIL_BODY(p.M, SWEEP_CPU_VRZ3D_BACKWARD_CASE);
    #undef SWEEP_CPU_VRZ3D_BACKWARD_CASE
}
...
{
    TORCH_CHECK(engine::is_cpu_input(in), "sweep_cpu::acoustic_vrz3d::forward called with non-CPU tensors");
    if (!can_use_acoustic_vrz3d_raw_forward(in)) return engine::forward(in, EquationKind::AcousticVRZ3D);
    auto result = forward_acoustic_vrz3d_raw(in);
    ForwardOutput out;
...
{
    TORCH_CHECK(engine::is_cpu_input(in), "sweep_cpu::acoustic_vrz3d::backward called with non-CPU tensors");
    TORCH_CHECK(can_use_acoustic_vrz3d_raw_backward(in),
                "AcousticVRZ3D CPU backward requires the handwritten raw float32 path; unsupported inputs will not fall back to torch autograd.");
    return backward_acoustic_vrz3d_raw(in);
}
