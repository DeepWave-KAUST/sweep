...
ForwardOutput forward(const ForwardInput& in)
{
    ...
    if (p.use_checkpoint) {
        ...
        if (p.use_recursive_checkpoint) {
            ...
            TORCH_CHECK(p.checkpoint_steps.dim() == 1,
                        "checkpoint_steps must be 1-D");
        } else {
    ...
    if (staged_boundary)
        boundary_saver.allocate(p.use_boundary_saving, 2, 5, solver, vp, save_width, 1, true, false,
                                p.transfer_interval, p.boundary_cpu, p.boundary_gpu, p.last_two,
                                p.use_pinned_memory);
    else
        boundary_saver.allocate(p.use_boundary_saving, 2, 5, solver, vp, save_width, 1, true, true,
    ...
    CheckpointRuntime checkpoint_runtime(
        p.checkpoints, 15, p.use_checkpoint, p.use_recursive_checkpoint,
        p.checkpoint_interval, p.checkpoint_steps, p.checkpoint_on_cpu,
        "forward", "elastic_vr2d"
    );
