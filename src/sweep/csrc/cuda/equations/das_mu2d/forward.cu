...
ForwardOutput forward(const ForwardInput& in)
{
    ...
    DasMuWavefieldTensor2D wavefield;
    ...
    if (staged_boundary)
        boundary_saver.allocate(p.use_boundary_saving, 2, 8, solver, vp, save_width, 1, true, false, p.transfer_interval, p.boundary_cpu, p.boundary_gpu, p.last_two, p.use_pinned_memory);
    else
        boundary_saver.allocate(p.use_boundary_saving, 2, 8, solver, vp, save_width, 1, true, true, 1, {}, p.boundary_gpu, p.last_two, p.use_pinned_memory);
    ...
    for (unsigned int it = 0; it < p.nt; ++it) {
        ...
        for (int isrc = 0; isrc < nsrc_fields; ++isrc) {
            float* field = das_mu2d_field_ptr(wf, source_fields[isrc].item<int>());
        ...
        for (int irec = 0; irec < nrec_fields; ++irec) {
            float* field = das_mu2d_field_ptr(wf, receiver_fields[irec].item<int>());
            if (field == nullptr) continue;
    ...
    if (p.use_boundary_saving) {
        ...
        boundary_saver.last_two_t.select(0,5).select(0,0).copy_(wavefield.exx_t);
        boundary_saver.last_two_t.select(0,6).select(0,0).copy_(wavefield.ezz_t);
        boundary_saver.last_two_t.select(0,7).select(0,0).copy_(wavefield.exz_t);
    }
