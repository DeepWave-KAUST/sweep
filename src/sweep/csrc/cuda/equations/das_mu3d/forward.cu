...
ForwardOutput forward(const ForwardInput& in)
{
    ...
    DasMuWavefieldTensor3D wavefield;
    if (!p.wavefields.empty())
        wavefield.bind(p.wavefields, true);
    else
        wavefield.allocate(vp, true);
    auto wf = wavefield.view();
    auto elastic_wf = wavefield.elastic_view();
    ...
    if (p.use_checkpoint) {
        TORCH_CHECK(p.checkpoints.size() == 33, "DAS Mu 3D checkpointing expects 33 checkpoint tensors");
    ...
    boundary_saver.allocate(
        p.use_boundary_saving, 3, 15, solver, vp, save_width, 1,
    ...
    for (unsigned int it = 0; it < p.nt; ++it) {
        float* u_this_t = u_allt.defined() ? u_allt[it].data_ptr<float>() : nullptr;
        ...
        for (int isrc = 0; isrc < nsrc_fields; ++isrc) {
            float* field = das_mu3d_field_ptr(wf, source_fields[isrc].item<int>());
        ...
        if (p.use_boundary_saving) {
            float* fields[15] = {
                ...
                wf.exx, wf.eyy, wf.ezz, wf.exy, wf.exz, wf.eyz
            };
        ...
        for (int irec = 0; irec < nrec_fields; ++irec) {
            float* field = das_mu3d_field_ptr(wf, receiver_fields[irec].item<int>());
            if (field == nullptr) continue;
    ...
    if (p.use_boundary_saving) {
        ...
        boundary_saver.last_two_t.select(0, 9).select(0, 0).copy_(wavefield.exx_t);
        boundary_saver.last_two_t.select(0, 10).select(0, 0).copy_(wavefield.eyy_t);
        boundary_saver.last_two_t.select(0, 11).select(0, 0).copy_(wavefield.ezz_t);
        boundary_saver.last_two_t.select(0, 12).select(0, 0).copy_(wavefield.exy_t);
        boundary_saver.last_two_t.select(0, 13).select(0, 0).copy_(wavefield.exz_t);
        boundary_saver.last_two_t.select(0, 14).select(0, 0).copy_(wavefield.eyz_t);
    }
