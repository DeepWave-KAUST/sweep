...
void undo_body_force_source_injection_3d(
    ...
    torch::Tensor& grad_rho,
    DasMuWavefieldPointer3D& adj_view,
    const torch::Tensor& rho,
    const BackwardInput& p,
...
)
{
    ...
    for (int isrc = 0; isrc < source_fields.numel(); ++isrc) {
        ...
        if (sfield > 2) continue;                  // velocity components only
        float* adj_field = das_mu3d_field_ptr(adj_view, sfield);
...
void zero_das_mu3d_state(DasMuWavefieldTensor3D& wf)
{
    for (auto& tensor : wf.state_tensors()) {
...
void replay_forward_to_time_3d(
    const BackwardInput& p,
    DasMuWavefieldTensor3D& forward,
...
)
{
    ...
    for (int it = start_time; it < p.nt; ++it) {
        ...
        for (int isrc = 0; isrc < nsrc_fields; ++isrc) {
            float* field = das_mu3d_field_ptr(for_view, source_fields[isrc].item<int>());
...
void backward_segment_3d(
    ...
    const torch::Tensor& rho,
    DasMuWavefieldTensor3D& start_state,
    DasMuWavefieldTensor3D& adjoint,
    CheckpointRuntime& checkpoint_runtime,
    int start,
    int end,
...
BackwardOutput backward_bs(const BackwardInput& in)
{
    ...
    boundary_saver.allocate(
        true, 3, 15, solver, vp, save_width, 1,
