...
from geophyai.equations import AEC
...
model = RNNJax(AEC(spatial_order=spatial_order, backend='jax'), 
            shape=shape, 
            dev=dev, 
            abcn=abcn, 
            dh=dh,
...
plt.imshow(obs[-1][...,0], vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto')
plt.colorbar()
plt.tight_layout()
plt.savefig(f'{save_path}/elastic_p.png', dpi=300, bbox_inches='tight')
plt.close()
# exit()
...
# plt.close()
# vmin, vmax = np.percentile(obs[-1][...,2], [2, 98])
# plt.imshow(obs[-1].squeeze()[...,2], vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto')
# plt.colorbar()
# plt.tight_layout()
