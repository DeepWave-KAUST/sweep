...
def build_encoded_batch(wave, obs, shot_idx, cfg):
    standard_obs = to_standard_layout(obs, cfg)
    signs = np.random.choice(np.array([-1.0, 1.0], dtype=np.float32), size=(len(shot_idx), 1))
    encoded_wave = signs * wave[None, :]
    encoded_obs = np.sum(standard_obs[shot_idx] * signs[:, None, None, None], axis=0, keepdims=True)
