...
def main():
    ...
    def fwi_step(current_vp, current_vs, current_rho, shot_idx, enc_signs, current_wave, all_sources, all_receivers, shared_receivers, all_obs):
        def loss_fn(vp_param, vs_param, rho_param, shots, wavelet, sources_all, receivers_all, obs_all):
            if cfg["source_encoding"]:
                encoded_wave = enc_signs * jnp.broadcast_to(wavelet, (shots.shape[0], wavelet.shape[0]))
                encoded_obs = jnp.sum(obs_all[shots] * enc_signs[:, None, None, None], axis=0, keepdims=True)
                syn = solver(
                    encoded_wave,
                    sources=sources_all[shots],
                    receivers=shared_receivers,
                    source_encoding=True,
                    models=[vp_param, vs_param, rho_param],
                )
                return jnp.sum((syn - encoded_obs) ** 2)

            syn = solver(
                wavelet,
                sources=sources_all[shots],
                receivers=receivers_all[shots],
                models=[vp_param, vs_param, rho_param],
            )
            return jnp.sum((syn - obs_all[shots]) ** 2)
    ...
    for epoch in tqdm.trange(cfg["epochs"]):
        ...
        shot_idx = random.choice(subkey, nshots, shape=(batchsize,), replace=False)
        signs = jnp.where(
            random.bernoulli(subkey, p=0.5, shape=(batchsize, 1)),
            jnp.float32(-1.0),
            jnp.float32(1.0),
        )

        loss, grads = fwi_step(vp, vs, rho, shot_idx, signs, wave_jax, sources_jax, receivers_jax, receivers_shared, obs_jax)
        vp, opt_states[0] = update_vp(vp, grads[0], opt_states[0])
        vs, opt_states[1] = update_vs(vs, grads[1], opt_states[1])
        rho, opt_states[2] = update_rho(rho, grads[2], opt_states[2])
