...
def percentile_clip(data, percentile=99.5):
    values = np.abs(np.asarray(data))
    finite = values[np.isfinite(values)]
    if finite.size == 0:
        return 1e-6
    amp = np.percentile(finite, percentile)
...
def plot_record_grid(records, titles, out_path, figure_title):
    ...
    for ax, record, title in zip(axes[0], records, titles):
        vmax = percentile_clip(record)
        ax.imshow(record, cmap="seismic", aspect="auto", vmin=-vmax, vmax=vmax)
        ax.set_title(title)
