...
def make_inputs(nt, shape):
    ...
    sources = np.array([[nx // 2, SOURCE_Z]], dtype=np.int64)
    receivers = np.stack(
        [np.arange(12, nx - 12, dtype=np.int64), np.full(nx - 24, RECEIVER_Z, dtype=np.int64)],
        axis=1,
    )[None, ...]
...
def plot_model_overview(models, sources, receivers, out_path):
    names = ["vp", "vs", "rho"]
    units = ["Velocity (m/s)", "Velocity (m/s)", "Density (kg/m^3)"]
    arrays = [model.detach().cpu().numpy() if isinstance(model, torch.Tensor) else np.asarray(model) for model in models]

    fig, axes = plt.subplots(1, len(arrays), figsize=(15, 4), squeeze=False)
    for ax, name, unit, data in zip(axes[0], names, units, arrays):
...
def plot_wavefield_comparison(fs_snapshots, abs_snapshots, times, out_path):
    ...
    for row, name in enumerate(names):
        ...
        for col, it in enumerate(times):
            ...
            abs_ax.set_title(f"{name} absorbing boundary t={it}")
...
def plot_top_boundary(fs_snapshots, abs_snapshots, times, out_path):
    ...
    panels = [
        ...
        ("Absorbing boundary top szz", abs_snapshots[:, 3, 0, :]),
        ("FS top sxz", fs_snapshots[:, 4, 0, :]),
        ("Absorbing boundary top sxz", abs_snapshots[:, 4, 0, :]),
    ]
...
def plot_record_comparison(fs_record, abs_record, out_path):
    ...
    fs_im = axes[0, 0].imshow(fs_panel, cmap="seismic", aspect="auto", vmin=-amp, vmax=amp)
    axes[0, 0].set_title("Receiver vz with free surface")
    fig.colorbar(fs_im, ax=axes[0, 0], shrink=0.8, label="Amplitude")
    abs_im = axes[0, 1].imshow(abs_panel, cmap="seismic", aspect="auto", vmin=-amp, vmax=amp)
    axes[0, 1].set_title("Receiver vz with absorbing boundary")
    fig.colorbar(abs_im, ax=axes[0, 1], shrink=0.8, label="Amplitude")
    diff_im = axes[0, 2].imshow(diff_panel, cmap="seismic", aspect="auto", vmin=-diff_amp, vmax=diff_amp)
    axes[0, 2].set_title("Difference")
    fig.colorbar(diff_im, ax=axes[0, 2], shrink=0.8, label="Amplitude")
...
def main():
    _, sources, receivers, models = make_inputs(NT, PHYS_SHAPE)
    plot_model_overview(models, sources, receivers, OUTPUT_DIR / "elastic_model_overview.png")
