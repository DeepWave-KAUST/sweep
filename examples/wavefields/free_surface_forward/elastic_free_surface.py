...
def build_inputs():
    nz, nx = PHYS_SHAPE
    vp = torch.full((nz, nx), 2400.0, dtype=torch.float32)
    vs = torch.full((nz, nx), 1200.0, dtype=torch.float32)
    rho = torch.full((nz, nx), 2000.0, dtype=torch.float32)
    t = np.arange(NT, dtype=np.float32) * DT - 0.12
    wavelet = 5e6 * ricker(t, f=DOM_FREQ).astype(np.float32)
...
def run_case(free_surface):
    ...
    vz_panels = [
        crop_panel(snapshots[i, 1, 0, 0].detach().cpu().numpy(), free_surface)
        for i in range(len(SNAPSHOT_TIMES))
    ]
    record_panel = to_record_panel(record)
    return vz_panels, record_panel, sources, receivers, models[0].numpy()
...
def main():
    ...
    plot_model(
        ...
        sources,
        receivers,
        OUTPUT_DIR / "elastic_model.png",
        "Elastic Free-Surface Test Model",
        DH,
        "Vp (m/s)",
    )
    plot_panel_grid(
        ...
        SNAPSHOT_TIMES,
        OUTPUT_DIR / "elastic_free_surface_snapshots_vz.png",
        "Elastic Forward Vz Snapshots",
        DH,
    )
    plot_record_comparison(
        [fs_record, abs_record, fs_record - abs_record],
        ["Free Surface", "Absorbing Top", "Residual"],
        OUTPUT_DIR / "elastic_free_surface_records_vz.png",
        "Elastic Vz Seismogram Comparison",
    )
    print(f"Saved elastic free-surface figures to {OUTPUT_DIR}")
