...
from geophyai.equations import AcousticVRR, Acoustic1st, AcousticVRZ
...
avrr = RNNJax(AcousticVRR(**eq_kwargs), source_type=['h1'], receiver_type=['h1'], **solver_kwargs)
avrz = RNNJax(AcousticVRZ(**eq_kwargs), source_type=['h1'], receiver_type=['h1'], **solver_kwargs)
a1st = RNNJax(Acoustic1st(**eq_kwargs), source_type=['p'], receiver_type=['p'], **solver_kwargs)

# Set the true a1stb
avrr.set_parameters([jnp.array(true_vp), jnp.array(true_rx), jnp.array(true_rz)])
avrz.set_parameters([jnp.array(true_vp), jnp.array(true_z)])
...
for solver, title in zip([avrr, a1st, avrz], ['Acoustic VRR', 'Acoustic 1st', 'Acoustic VRZ']):
