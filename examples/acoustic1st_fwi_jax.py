...
from geophyai.equations import Acoustic1st
from geophyai.signal import ricker
from functools import partial
import matplotlib.pyplot as plt
from configure import *

save_path = 'acoustic1st_fwi_l2_jax'
...
model = RNNJax(Acoustic1st(spatial_order=spatial_order, backend='jax'), 
                shape=shape, 
                dev=None,
                dh=dh,
                dt=dt,
...
# Set the true model
model.set_parameters([jnp.array(true_model), jnp.array(np.ones_like(true_model))])
...
# Set the model
model.set_parameters([jnp.array(smooth_model), jnp.array(np.ones_like(smooth_model))])
...
@jax.jit
def fwi_step(vp, rho, rand_shots):
    # @jax.jit
    def loss_fn(vp, rho, shot_nums):
        # Forward modeling
        syn = model(wave,
    ...
    (loss, data), gradients = jax.value_and_grad(loss_fn, argnums=(0,1),has_aux=True)(vp, rho, rand_shots)
    return loss, gradients[0]
