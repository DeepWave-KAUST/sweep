...
from geophyai.equations import AECLSRTM, AEC
...
solver = RNNJax(AEC(spatial_order=spatial_order, backend='jax'), 
            shape=shape, 
            dev=dev, 
            abcn=abcn, 
            dh=dh,
...
born = RNNJax(AECLSRTM(spatial_order=spatial_order, backend='jax'), 
            shape=shape, 
            dev=dev, 
            abcn=abcn, 
            dh=dh,
...
plt.colorbar()
plt.tight_layout()
plt.savefig(f'{save_path}/aec_p.png', dpi=300, bbox_inches='tight')
plt.close()
