...
def fwi_step(vp, vs, rho, rand_shots):
    ...
    f = partial(model, wavelet=wave, sources=sources[rand_shots], receivers=receivers[0:1], source_encoding=True)
    
    def loss_fn(*args):
        # Forward modeling
        syn = f(models=[p.value for p in args])
        return criterion(syn, jnp.sum(obs[rand_shots], axis=0)) # Must be a scalar
    
    # Compute the gradient
    models = [JaxTensor(p) for p in [vp, vs, rho]]

    loss = Loss(loss_fn, *models)
    loss.backward()

    return loss.value, [p.grad for p in models]
