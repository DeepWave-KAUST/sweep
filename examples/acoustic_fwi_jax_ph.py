...
save_path = 'acoustic_fwi_l2_jax_ph'
...
def fwd_base(*args):
    return model.equation.func_jax(*args[:-2])

def fwd(*args):
    wavefields, vjp_fun = jax.vjp(model.equation.func_jax, *args[:-2])
...
tmp1 = model.pad(jnp.zeros_like(model.vp))
tmp2 = model.pad(jnp.zeros_like(model.vp))
...
for epoch in tqdm.trange(epochs):
    ...
    model.vp, opt_state = update_fn(model.vp, grads[0], opt_state)
    ...
    if epoch % show_every == 0:
        ...
        vmin,vmax=np.percentile(grads[0], [2, 98])
        ax[2].imshow(grads[0], vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto', extent=extent)
        plt.tight_layout()
        ...
        ax = axes[0].imshow(grads[1], vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto', extent=extent)
        plt.colorbar(ax, ax=axes[0], orientation='vertical')
        axes[0].set_title('Source-side Illumination')
        vmin, vmax = np.percentile(grads[2], [2, 98])
        ax = axes[1].imshow(grads[2], vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto', extent=extent)
        plt.colorbar(ax, ax=axes[1], orientation='vertical')
        axes[1].set_title('Receiver-side Illumination')
        plt.tight_layout()
        plt.savefig(f'{save_path}/ill_{epoch}.png', dpi=300, bbox_inches='tight')
        plt.close()