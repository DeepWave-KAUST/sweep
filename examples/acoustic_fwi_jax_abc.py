...
save_path = 'acoustic_fwi_l2_jax_abc'
...
model_pml = RNNJax(Acoustic(spatial_order=spatial_order, backend='jax'), 
                shape=shape, 
                dev=None,
                dh=dh,
                dt=dt,
...
# Set the model
model_pml.set_parameters([jnp.array(smooth_model)])
opt_state = opt.init(model_pml.vp)
...
for epoch in tqdm.trange(epochs):
    ...
    loss, grads = fwi_step(model_pml.vp, rand_shots)
    model_pml.vp, opt_state = update_fn(model_pml.vp, grads, opt_state)
    ...
    if epoch % show_every == 0:
        ...
        ax[1].imshow(model_pml.vp, vmin=vmin, vmax=vmax, cmap='seismic', aspect='auto', extent=extent)
