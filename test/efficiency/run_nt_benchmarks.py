...
DEFAULT_NT_VALUES = (300, 600, 900, 1200, 1800, 2400)


def build_parser():
    parser = argparse.ArgumentParser(
        description="Run CUDA/Deepwave benchmark scripts across nt values and collect mean/std."
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "benchmark_nt_<mode>_summary.csv in this directory."
        ),
    )
    parser.add_argument(
        "--nt-values",
        nargs="+",
        type=int,
        default=DEFAULT_NT_VALUES,
        metavar="NT",
        help=(
            "Time-step counts to benchmark. Defaults to: "
            + ", ".join(str(value) for value in DEFAULT_NT_VALUES)
        ),
    )
    parser.add_argument(
        "script_args",
...
def main():
    ...
    for job in jobs:
        ...
        print(f"\nRunning {backend} (nt={nt}): {' '.join(cmd)}")
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"{backend} benchmark for nt={nt} failed with exit code {result.returncode}.")
    ...
    output_path = Path(args.output) if args.output else base_dir / f"benchmark_nt_{args.mode}_summary.csv"
