...
DEFAULT_NSHOTS = (1, 2, 4, 8, 16, 32)


def build_parser():
    parser = argparse.ArgumentParser(
        description="Run CUDA/Deepwave benchmark scripts across shot counts and collect mean/std."
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "benchmark_nshots_<mode>_summary.csv in this directory."
        ),
    )
    parser.add_argument(
        "--nshots-values",
        nargs="+",
        type=int,
        default=DEFAULT_NSHOTS,
        metavar="NSHOTS",
        help=(
            "Shot counts to benchmark. Defaults to: "
            + ", ".join(str(value) for value in DEFAULT_NSHOTS)
        ),
    )
    parser.add_argument(
        "script_args",
...
def main():
    ...
    run_params = {
        ...
        "nshots_values": list(args.nshots_values),
    }

    jobs = []
    for backend in BACKENDS:
        script = base_dir / f"acoustic_{args.mode}_{backend}.py"
        for nshots in args.nshots_values:
            jobs.append(
                {
                    "backend": backend,
                    "variant": f"nshots_{nshots}",
                    "script": script,
                    "extra_args": ["--nshots", str(nshots)],
                    "nshots": nshots,
                }
            )
    ...
    for job in jobs:
        ...
        print(f"\nRunning {backend} (nshots={nshots}): {' '.join(cmd)}")
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"{backend} benchmark for nshots={nshots} failed with exit code {result.returncode}."
            )
    ...
    output_path = Path(args.output) if args.output else base_dir / f"benchmark_nshots_{args.mode}_summary.csv"
