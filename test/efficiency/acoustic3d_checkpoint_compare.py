...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Compare 3D acoustic CUDA full-wavefield, boundary saving, "
            "chunk checkpointing, recursive checkpointing, and PyTorch autograd "
            "in terms of gradient, timing, and peak memory."
        )
    )
    ...
    parser.add_argument("--checkpoint-chunks", type=parse_int_list, default=(100,))
    ...
    parser.add_argument("--output", default="acoustic3d_checkpoint_compare.png")
...
def maybe_plot(results, output_path):
    ...
    for name in row_names:
        ...
        slice_stack.append(grad[grad.shape[0] // 2])
        slice_stack.append(grad[:, grad.shape[1] // 2])
        slice_stack.append(grad[:, :, grad.shape[2] // 2])
    slice_stack = np.stack(slice_stack, axis=0)
    vmin, vmax = np.percentile(slice_stack, [1.0, 99.0])
    ...
    for row_idx, name in enumerate(row_names):
        ...
        ax_z = axes[row_idx, 0]
        im_z = ax_z.imshow(grad[mid_z], cmap="seismic", vmin=vmin, vmax=vmax, aspect="auto")
        ax_z.set_title(
            f"{name} z-slice | mean {results[name]['elapsed_s'] * 1000.0:.2f} ms | "
            f"peak {results[name]['peak_memory_mib']:.2f} MiB"
        )
        fig.colorbar(im_z, ax=ax_z)

        ax_y = axes[row_idx, 1]
        im_y = ax_y.imshow(grad[:, mid_y], cmap="seismic", vmin=vmin, vmax=vmax, aspect="auto")
        ax_y.set_title(f"{name} y-slice")
        fig.colorbar(im_y, ax=ax_y)

        ax_x = axes[row_idx, 2]
        im_x = ax_x.imshow(grad[:, :, mid_x], cmap="seismic", vmin=vmin, vmax=vmax, aspect="auto")
        ax_x.set_title(f"{name} x-slice")
...
def maybe_plot_checkpoint_sweeps(chunk_results, recursive_results, output_path):
    ...
    axes[0, 0].set_title("3D Chunk Checkpoint Timing")
    axes[0, 0].grid(True, alpha=0.3)
    axes[1, 0].set_xlabel("checkpoint_chunks")
    axes[1, 0].set_ylabel("peak memory (MiB)")
    axes[1, 0].set_title("3D Chunk Checkpoint Peak Memory")
    ...
    axes[0, 1].set_title("3D Recursive Checkpoint Timing")
    axes[0, 1].grid(True, alpha=0.3)
    axes[1, 1].set_xlabel("checkpoint_count")
    axes[1, 1].set_ylabel("peak memory (MiB)")
    axes[1, 1].set_title("3D Recursive Checkpoint Peak Memory")
...
def main():
    ...
    true_vp = np.full((args.nz, args.ny, args.nx), 1500.0, dtype=np.float32)
    true_vp[args.nz // 2 :, :, :] = 2000.0
    ...
    receivers = np.stack(
        (rec_x.reshape(-1), rec_y.reshape(-1), rec_z),
        axis=1,
    )[None, ...]
    ...
    print("Sources:\n", sources)
    print("Receivers:", receivers.shape[1])
