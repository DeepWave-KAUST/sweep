...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Benchmark 2D acoustic inversion/gradient computation for Deepwave."
    )
...
def main():
    ...
    def build_step():
        vp = torch.from_numpy(case["vp"].T.copy()).to(device).requires_grad_(True)
    ...
    print("\nInversion benchmark")
    run_benchmark("Deepwave", build_step, sync, args.warmup, args.repeats)
