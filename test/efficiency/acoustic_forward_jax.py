...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Benchmark 2D acoustic forward modeling for the JAX backend."
    )
...
def main():
    ...
    def build_step():
        ...
        def run():
            record = solver(wave, sources, receivers, models=[vp])
            return record
    ...
    append_summary_csv(
        Path(__file__).resolve().parent / "benchmark_forward_summary.csv",
        {
            "backend": "jax",
            "variant": "default",
            "label": "JAX",
