...
DEFAULT_RECEIVER_STRIDES = (16, 8, 4, 2, 1)


def build_parser():
    parser = argparse.ArgumentParser(
        description="Run CUDA/Deepwave benchmark scripts across receiver densities and collect mean/std."
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "benchmark_nreceivers_<mode>_summary.csv in this directory."
        ),
    )
    parser.add_argument(
        "--receiver-strides",
        nargs="+",
        type=int,
        default=DEFAULT_RECEIVER_STRIDES,
        metavar="STRIDE",
        help=(
            "Receiver strides to benchmark. Smaller stride means more receivers. Defaults to: "
            + ", ".join(str(value) for value in DEFAULT_RECEIVER_STRIDES)
        ),
    )
    parser.add_argument(
        "script_args",
...
def format_row(row):
    return (
        f"{row['backend']:>10} | "
        f"stride {row['receiver_stride']:<2} | "
        f"receivers {row['nreceivers']:<4} | "
...
def main():
    ...
    run_params = {
        ...
        "receiver_strides": list(args.receiver_strides),
    }

    jobs = []
    for backend in BACKENDS:
        script = base_dir / f"acoustic_{args.mode}_{backend}.py"
        for stride in args.receiver_strides:
            jobs.append(
                {
                    "backend": backend,
                    "variant": f"stride_{stride}",
                    "script": script,
                    "extra_args": ["--receiver-stride", str(stride)],
                    "receiver_stride": stride,
                }
            )
    ...
    for job in jobs:
        backend = job["backend"]
        script = job["script"]
        stride = job["receiver_stride"]
        cmd = [args.python, script.name, *job["extra_args"], *forwarded]
        print(f"\nRunning {backend} (receiver_stride={stride}): {' '.join(cmd)}")
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"{backend} benchmark for receiver_stride={stride} failed with exit code {result.returncode}."
            )
    ...
    output_path = (
        Path(args.output)
        if args.output
        else base_dir / f"benchmark_nreceivers_{args.mode}_summary.csv"
    )
