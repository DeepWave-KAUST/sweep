...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Compare Elastic 2D/3D gradients from CUDA full, checkpointing, or boundary saving against PyTorch."
    )
...
def build_solvers(args, device):
    if args.dim == "2d":
        equation = Elastic(spatial_order=args.spatial_order, device=device)
        shape = (args.nz, args.nx)
        source_type = ["sxx", "szz"]
        receiver_type = ["vx", "vz"]
    else:
        equation = Elastic3D(spatial_order=args.spatial_order, device=device)
...
def run_case(name, solver, models_np, wave, sources, receivers, device):
    ...
    return {
        ...
        "grads": {
            "vp": models[0].grad.detach().cpu().numpy().copy(),
            "vs": models[1].grad.detach().cpu().numpy().copy(),
            "rho": models[2].grad.detach().cpu().numpy().copy(),
        },
    }
...
def plot_results(results, output_path, dim):
    ...
    row_names = (cuda_name, "torch", "cuda-torch")
    fig, axes = plt.subplots(len(row_names), len(params), figsize=(5 * len(params), 4 * len(row_names)))

    for col_idx, param in enumerate(params):
        cuda_grad = grad_slice(results[cuda_name]["grads"][param], dim)
        torch_grad = grad_slice(results["torch"]["grads"][param], dim)
        row_images = {
            cuda_name: cuda_grad,
            "torch": torch_grad,
            "cuda-torch": cuda_grad - torch_grad,
        }

        for row_idx, row_name in enumerate(row_names):
            ax = axes[row_idx, col_idx]
            image = row_images[row_name]
            finite = image[np.isfinite(image)]
            if finite.size == 0:
                vmin, vmax = -1.0, 1.0
            elif row_name.startswith("cuda_") or row_name == "torch":
...
def main(argv=None):
    ...
    for name, solver in solvers.items():
        try:
            results[name] = run_case(name, solver, models_np, wave, sources, receivers, device)
        except Exception:
            print(f"{name:>24} | failed")
            raise
        print(f"{name:>24} | loss {results[name]['loss']:.6e}")
        for param in ("vp", "vs", "rho"):
            print(" " * 26 + summarize_array(param, results[name]["grads"][param]))
